...
public class CSV_To_DB
{
	...
	{
        Connection conn = connect("C:/Users/Master/Downloads/SQLiteData.db"); // Connection is a object session
        // that establishes a connection to SQL databases, and the connect variable here is followed by the host local path to the storage point where the SQLLite stores DB files
	...
	public static void createT(Connection conn) // Creates the Table in using the SQLLite Library by passing in the connection argument from our Main Method
	{
		...
		try 
        {
			...
			String drop = "DROP TABLE IF EXISTS c";
   ...
   public static void add_To_DB(Connection conn, String csv) // Method for adding data by rows from the CSV to the DB, arguments passed are the connection object and our CSV
   {
	    ...
	    try 
	    {
            ...
            for(String line; (line = r.readLine()) != null;) 
            {
              ...
              pt.setString(1, (cols[0]));
              pt.setString(2, (cols[1])); 
              pt.setString(3, (cols[2]));
              pt.setString(4, (cols[3]));
              pt.setString(5, (cols[4]));
              pt.setString(6, (cols[5]));
              pt.setString(7, (cols[6]));
              pt.setString(8, (cols[7]));
              pt.setString(9, (cols[8]));
              pt.setString(10, (cols[9]));
